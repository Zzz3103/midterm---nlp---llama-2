...
class Attention(nn.Module):
    ...
    def compute_query_key_value_scores(self,
        ...
        attn = F.softmax(scores.float(), dim=-1).type_as(query)
        attn = self.attn_dropout(attn)
        output = torch.matmul(attn, value)
        return self.attn_dropout(output)
...
class LlamaLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        sa = self.attention(norm_x)
        sa_out = x + sa
        
        norm_sa_out = self.ffn_norm(sa_out)
        ffn = self.feed_forward(norm_sa_out)
